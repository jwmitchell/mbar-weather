...
# Get weather station data within 5 miles and up to 72 hours prior to ignition 
...
weather_db = weather_config.config[program_args.utility]['WEATHER_DB'] # Not necessary but why not
...
tm_offset = weather_config.config[program_args.utility]['TIME_OFFSET']  #-1 is prior, 0 is around, 1 is after
xl_datetime_column = weather_config.config[program_args.utility]['XL_DATETIME_COLUMN']
xl_latitude_column = weather_config.config[program_args.utility]['XL_LATITUDE_COLUMN']
xl_longitude_column = weather_config.config[program_args.utility]['XL_LONGITUDE_COLUMN']
...
#weather_station_cell = weather_config.config[program_args.utility]['WS'] No weather station
# Copy header row
