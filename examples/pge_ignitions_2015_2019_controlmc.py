...
# to accompany analyis done in 'pge_ignitions_2015_2019'.
#
# The routine randomly draws an ignition event and then picks a random time
# between January 1, 2015, 0:00 and December 31, 2019, 23:59:59. Maximum wind
...
nevents = int(weather_config.config['PGE']['EVENTS'])
frow = int(weather_config.config['PGE']['FIRST_ROW'])
lrow = int(weather_config.config['PGE']['LAST_ROW'])
offset = int(weather_config.config['PGE']['ROW_OFFSET']) or 0
start_date = weather_config.config['PGE']['START_DATE']
end_date = weather_config.config['PGE']['END_DATE']
...
for ievt in range(nevents):
    ...
    for icol in range(len(sht_all[irow])):
        sht_wind[jrow][icol].value = sht_all[irow][icol].value
    ...
    lat = sht_all["D"+srow].value
    lon = sht_all["E"+srow].value
    ...
    first_wg_cell = int(weather_config.config['PGE']['FREE_CELL'])
    
    # Clear wind gust cells
    
    nmgcells = len(gtpl)*len(ttpl)*5
    for ic in range(nmgcells):
        sht_wind[jrow][first_wg_cell+ic].value = None
    ...
    for it in max_gusts:
        for ig in it:
            for val in ig:
                sht_wind[jrow+offset][icell].value = val
                icell += 1
